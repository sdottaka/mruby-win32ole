...
STDOUT = Kernel unless Module.const_defined?(:STDOUT)
...
#   generate_propputref_methods(klass, io)
  ...
  def generate_constants(klass, io = STDOUT)
    klass.variables.select {|v|
      ...
      io.print "  "
      io.print v.name[0].upcase + v.name[1..-1]
      io.print " = "
      io.puts  v.value
    end
  end
  ...
  def class_name(klass)
    ...
    if !("ABCDEFGHIJKLMNOPQRSTUVWXYZ".include?(klass_name[0])) || Module.const_defined?(klass_name)
